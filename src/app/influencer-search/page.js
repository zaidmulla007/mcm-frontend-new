  ...
  const formattedPosts = lastPostsData.map(post => {
    ...
    if (post.isGrouped && post.coins) {
      ...
      return {
        ...
        educationalPurpose: post.educationalPurpose,
        actionableInsights: post.actionableInsights,
        marketingContent: post.marketingContent,
      };
    }
...
export default function InfluencerSearchPage() {
  ...
  // Get MCM scoring directly from the post data (available in 3days-all-posts response)
  const getMcmScoring = useCallback((rec, outerPost) => {
    const source = rec || outerPost;
    if (!source) return null;
    // Check if scoring data exists in the post object
    if (source.overallScore != null || source.educationalPurpose != null || source.actionableInsights != null) {
      return {
        overallScore: source.overallScore || 0,
        educationalPurpose: source.educationalPurpose || 0,
        actionableInsights: source.actionableInsights || 0,
        marketingContent: source.marketingContent,
      };
    }
    // Fallback to outer post if rec didn't have it
    if (outerPost && outerPost !== source && (outerPost.overallScore != null || outerPost.educationalPurpose != null || outerPost.actionableInsights != null)) {
      return {
        overallScore: outerPost.overallScore || 0,
        educationalPurpose: outerPost.educationalPurpose || 0,
        actionableInsights: outerPost.actionableInsights || 0,
        marketingContent: outerPost.marketingContent,
      };
    }
    return null;
  }, []);

  // Render stars for MCM scoring (converts 0-10 score to 5-star rating)
  const renderMcmStars = (score) => {
    const rating = (score || 0) / 2;
        ...
        lastPostsData.results.forEach(item => {
          if (item.channelID === channelId && item.mentioned && Array.isArray(item.mentioned)) {
            ...
            posts.push({
              ...
              educationalPurpose: item.educationalPurpose,
              actionableInsights: item.actionableInsights,
      ...
      <main className="mx-auto px-4 pb-8 overflow-x-hidden relative z-10">
        <div className="min-w-0">
          ...
          <div className="bg-white rounded-3xl shadow-md">
            ...
            <div className="flex flex-col items-start px-4 py-2 border-b border-indigo-200/30 bg-gradient-to-r from-cyan-50/50 to-fuchsia-50/50 backdrop-blur-sm">
            ...
            <div>
              <table className="w-full relative">
                <thead>
                  {/* Single header row */}
                  <tr className="bg-gradient-to-r from-cyan-500 to-indigo-500 border-b-2 border-white/20">
                    {/* Influencer Column with Platform Toggle */}
                    <th className="px-1 py-2 text-center text-[10px] font-bold text-white tracking-wider border-r border-white/20 w-44">
                    ...
                    <th className="px-1 py-2 text-center text-[10px] font-bold text-white tracking-wider border-r border-white/20 w-44">
                      ...
                      <span className="relative group cursor-pointer z-[9999]">
                        <span className="text-cyan-200 text-[9px]">ⓘ</span>
                    ...
                    <th className="px-0 py-2">
                      <div className="flex items-stretch w-full">
                        <div className="w-[10%] text-[10px] font-bold text-white text-center flex flex-col justify-center gap-1 px-1 border-r border-white/30">
                          Date & Time
                        </div>
                        <div className="w-[22%] text-[10px] font-bold text-white text-center flex items-stretch border-r border-white/30">
                          <div className="w-[50%] flex flex-col justify-center px-1 border-r border-white/30">
                            <span>Mentioned<br />Coins</span>
                          </div>
                          <div className="w-[50%] flex flex-col justify-center px-1">
                            <div className="flex items-center justify-center gap-1">
                              <span>Sentiment<br />ST/LT</span>
                              <span className="relative group cursor-pointer z-[9999]">
                                <span className="text-cyan-200 text-[9px]">ⓘ</span>
                        ...
                        <div className="w-[18%] text-[10px] font-bold text-white text-center flex flex-col justify-center px-1 border-r border-white/30">
                          <span>MCM Scoring</span>
                        </div>
                        <div className="w-[50%] flex flex-col items-center justify-center text-white text-center leading-tight px-1">
                          <div className="flex items-center">
                            <span className="text-[10px] font-bold mr-1">
                              Summary
                            </span>
                            {/* <div className="text-[10px] px-1.5 py-0.5 rounded-full font-bold tracking-wide bg-white/20 text-white-300 border border-white-300/50">
                              Ai
                            </div> */}
                          </div>
                ...
                <tbody className="bg-white divide-y divide-gray-200 relative" style={{ isolation: 'isolate' }}>
                  {pageLoading || initialLoad || (selectedPlatform === "youtube" && youtubeInfluencers.length === 0) || (selectedPlatform === "telegram" && telegramInfluencers.length === 0) ? (
                      ...
                      {paginatedPosts.map((post, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* <td className="px-1 py-1 border-r border-gray-200 group-hover:bg-white/50 transition-all duration-300">
                            ...
                            <td className="px-0 py-0 h-full" style={{ height: '1px' }}>
                                ...
                                {(() => {
                                  ...
                                  return displayedRecommendations.map((rec, idx) => {
                                    ...
                                    if (rec.isGrouped && rec.coins) {
                                      ...
                                      // Content text and word limit for summary
                                      ...
                                      const scoring = getMcmScoring(rec, post);

                                      return (
                                        <div key={`group-wrapper-${idx}`} className="flex items-stretch w-full border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors duration-200 flex-1">
                                          {/* Date and Time Column */}
                                          <div className="w-[10%] flex flex-col items-center justify-center gap-1 px-1 border-r border-gray-200 py-2 self-stretch">
                                            {rec.date && rec.time ? (
                                              <>
                                          ...
                                          {/* Mentioned Coins + Sentiment ST/LT Columns */}
                                          <div className="w-[22%] flex flex-col border-r border-gray-200 self-stretch">
                                            {visibleCoins.map((coinData, coinIdx) => (
                                              <div key={`coin-${idx}-${coinIdx}`} className={`flex items-stretch flex-1 ${coinIdx < visibleCoins.length - 1 ? 'border-b border-gray-200' : ''}`}>
                                                {/* Coin */}
                                                <div className="w-[50%] flex items-center gap-0.5 px-1 py-1.5 border-r border-gray-200">
                                                  <div
                                                ...
                                                {/* Sentiment */}
                                                <div className="w-[50%] flex items-center justify-center px-1 py-1.5">
                                                  <div
                                                    className={`px-2 py-0.5 rounded-full flex items-center gap-1 text-[7px] font-semibold ${coinData.type === "bullish"
                                            ...
                                            {totalCoins > 3 && (
                                              <div className="flex justify-center items-center py-1 w-full">
                                                <button
                                                  ...
                                                  title={showingAllCoins ? "Show less" : `Show more (${totalCoins - visibleCoinsCount} remaining)`}
                                                >
                                          ...
                                          {/* MCM Scoring Column */}
                                          <div className="w-[18%] flex items-center px-2 border-r border-gray-200 py-2 self-stretch">
                                            {scoring ? (
                                              <ul className="text-[9px] space-y-1 w-full">
                                                <li className="flex items-center justify-between gap-2">
                                                  <span className="text-gray-700 whitespace-nowrap">Overall</span>
                                                  {renderMcmStars(scoring.overallScore)}
                                                </li>
                                                <li className="flex items-center justify-between gap-2">
                                                  <span className="text-gray-700 whitespace-nowrap">Educational</span>
                                                  {renderMcmStars(scoring.educationalPurpose)}
                                                </li>
                                                <li className="flex items-center justify-between gap-2">
                                                  <span className="text-gray-700 whitespace-nowrap">Actionable</span>
                                                  {renderMcmStars(scoring.actionableInsights)}
                                                </li>
                                                <li className="flex items-center justify-between gap-2">
                                                  <span className="text-gray-700 whitespace-nowrap">Marketing</span>
                                                  {typeof scoring.marketingContent === "string" &&
                                                    scoring.marketingContent.toLowerCase().includes("no marketing content") ? (
                                                    <span className="text-gray-700 text-[8px]">None</span>
                                                  ) : scoring.marketingContent ? (
                                                    <div className="flex items-center gap-1">
                                                      <span className="text-gray-700 text-[8px]">Yes</span>
                                                      <span className="relative group/mkt cursor-pointer">
                                                        <span className="text-blue-600 text-[9px]">ⓘ</span>
                                                        <span className="invisible group-hover/mkt:visible absolute bottom-full mb-2 right-0 bg-gray-800 text-white text-xs p-3 rounded-lg shadow-xl w-64 break-words z-[9999]">
                                                          {scoring.marketingContent}
                                                        </span>
                                                      </span>
                                                    </div>
                                          ...
                                          {/* Summary/Title Column */}
                                          <div className="w-[50%] flex items-start px-2 py-2 self-stretch">
                                            {contentText ? (
                                              <div className="text-[9px] text-gray-700 leading-[1.4]" style={{
                                                ...
                                                <p className="mb-0">
                                                  ...
                                                  >
                                                    {showReadMore ? truncateText(contentText, wordLimit) : contentText}
                                                    {showReadMore && '...'}
                                                  </span>
                                                </p>
                                                {showReadMore && (
                                    ...
                                    return (
                                      <div key={idx} className={`flex items-stretch w-full border-b border-gray-200 last:border-b-0 hover:bg-gray-50 transition-colors duration-200 flex-1`}>
                                        <div className="flex flex-col items-center justify-center gap-1 w-[10%] px-1 border-r border-gray-200 py-2 self-stretch">
                                          {rec.date && rec.time ? (
                                            <>
                                              <span className="text-[8px] font-semibold text-black-900">
                                                {formatDate(rec.date)}
                                        ...
                                        {/* Mentioned Coin + Sentiment ST/LT */}
                                        <div className="w-[22%] flex items-stretch border-r border-gray-200 self-stretch">
                                          {/* Coin */}
                                          <div className="w-[50%] flex items-center gap-0.5 px-1 py-2 border-r border-gray-200">
                                            <div
                                              className="flex items-center gap-0.5 cursor-pointer hover:opacity-80 transition-opacity"
                                              onClick={(e) => {
                                                e.stopPropagation();
                                          ...
                                          {/* Sentiment */}
                                          <div className="w-[50%] flex items-center justify-center px-1 py-2">
                                            <div
                                              className={`px-2 py-0.5 rounded-full flex items-center gap-1 ${rec.type === "bullish"
                                                ? "bg-green-100 text-green-700"
                                                : "bg-red-100 text-red-700"
                                                }`}
                                            >
                                        ...
                                        <div className="w-[18%] flex items-center px-2 border-r border-gray-200 py-2 self-stretch">
                                          {(() => {
                                            const scoring = getMcmScoring(rec, post);
                                            if (!scoring) {
                                              return <span className="text-[8px] text-gray-400">N/A</span>;
                                            }
                                            return (
                                              <ul className="text-[9px] space-y-1 w-full">
                                                <li className="flex items-center justify-between gap-2">
                                        ...
                                        {/* Summary/Title Column */}
                                        <div className="w-[50%] flex justify-start break-words overflow-hidden px-2 py-2 self-stretch">
                                          {contentText ? (
                                            <div className="text-[8px] text-gray-600 break-words capitalize text-justify">
                                              <span
                                                onClick={(e) => {
                                                  e.stopPropagation();
                                                  const infId = influencer?.channel_id || influencer?.id || post.channelID;
      ...
      <AnimatePresence>
        {summaryModal.isOpen && (
          ...
          >
            ...
            >
              {/* Modal Header */}
              <div className="px-6 py-4 border-b border-gray-200 bg-gradient-to-r from-cyan-500 to-indigo-500">
              ...
              <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                <button
                  onClick={() => setSummaryModal({ isOpen: false, content: '', title: '' })}
                  className="w-full px-4 py-2 bg-gradient-to-r from-cyan-500 to-indigo-500 text-white font-semibold rounded-lg hover:opacity-90 transition-opacity"
                >
                  Close
                </button>
              </div>
