...
const LeftPanel = ({ influencer, failedImages }) => {
    ...
    const imageUrl = influencer.channel_thumbnails?.high?.url;
    const showImage = imageUrl && !failedImages.has(imageUrl);
                ...
                <div className="w-full max-w-[180px] bg-white rounded-lg p-2 shadow-md border border-blue-100 mb-2">
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] font-bold text-gray-800">
                            {influencer.last_post_date_string || '--'} {influencer.last_post_time || ''}
                        </span>
                    </div>
                </div>
                                ...
                                {Object.entries(influencer.yearly_post_counts)
                                    ...
                                    .map(([year, count]) => (
                                        <div key={year} className="bg-white rounded-md p-1.5 border border-blue-50 text-center shadow-sm">
                                            <div className="text-[7px] text-gray-800 font-bold mb-0.5">{year}</div>
...
const RightPanel = ({ influencer, selectedTimeframe, timeframeOptions }) => {
    // Local timeframe state for MCM Rating only (doesn't affect influencer list filtering)
    const [mcmTimeframe, setMcmTimeframe] = useState(selectedTimeframe);
    ...
    const timeframeKey = getTimeframeKey(mcmTimeframe);
    ...
    const formatROI = (value) => {
        if (value === undefined || value === null || value === 0 || isNaN(value) || !isFinite(value)) return '--';
    ...
    const formatWinRate = (value, yearHas100) => {
    ...
    const getROIColor = (value) => {
        if (value === undefined || value === null || value === 0 || isNaN(value) || !isFinite(value)) return 'text-gray-400';
                ...
                <div className="flex-1 overflow-y-auto overflow-x-auto pr-1 space-y-3 custom-scrollbar min-h-0 py-1">
                    ...
                    <div className="bg-gradient-to-br from-purple-50 to-blue-50/50 rounded-lg p-2 shadow-sm border border-purple-100/50">
                        <div className="flex items-center justify-between mb-1">
                            ...
                            <select
                                value={mcmTimeframe}
                                onChange={(e) => setMcmTimeframe(e.target.value)}
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-center border-collapse">
                                ...
                                <tbody>
                                    {yearsData.length > 0 ? (
                                        yearsData.map(({ year, data }) => {
                                            const yearHas100 = timeframes.some(tf => data?.[tf.key]?.win_percentage === 100);
                                            return (
                                                <tr key={`${year}-win`} className="border-b border-gray-100 last:border-b-0 hover:bg-blue-50/30 transition-colors">
                                                    <td className="py-1.5 px-0.5 text-[7px] font-bold text-gray-900 text-left pl-1 border-r border-gray-200">{year}</td>
                                                    {timeframes.map((tf, idx) => {
                                                        const roi = data?.[tf.key]?.prob_weighted_returns;
                                                        const roiMissing = roi === undefined || roi === null || roi === 0 || isNaN(roi) || !isFinite(roi);
                                                        const winRate = roiMissing ? undefined : data?.[tf.key]?.win_percentage;
                                                        return (
                                                            <td key={tf.key} className={`py-1.5 px-0.5 text-[7px] font-bold whitespace-nowrap min-w-[32px] ${idx < timeframes.length - 1 ? 'border-r border-gray-100' : ''} ${getWinRateColor(winRate)}`}>
                                                                {formatWinRate(winRate, yearHas100)}
                                                            </td>
                                                        );
                                                    })}
                                                </tr>
...
const TOCLeftPanel = ({ influencers, selectedIndex, onSelect, onHoverSelect, selectedPlatform, setSelectedPlatform, searchQuery, setSearchQuery, selectedRating, setSelectedRating, failedImages, availableRatingOptions }) => {
    return (
        <div className="h-full w-full bg-white/95 backdrop-blur-sm flex flex-col p-3 md:p-4 relative border-r border-gray-100">
            ...
            <div className="relative flex flex-col z-10 h-full min-h-0">
                ...
                <div className="flex-shrink-0 mb-2 flex gap-1.5">
                    ...
                    >
                        {availableRatingOptions.map((option) => (
                            <option key={option.value} value={option.value}>
                    ...
                    {influencers.length > 0 ? influencers.map((inf, idx) => (
                        ...
                        >
                            <div className={`w-7 h-7 rounded-full overflow-hidden flex items-center justify-center font-bold text-xs shadow-md transition-transform flex-shrink-0 ${selectedIndex === idx ? 'scale-110 ring-2 ring-blue-400' : 'group-hover:scale-110'}`}>
                                {inf.channel_thumbnails?.high?.url && !failedImages.has(inf.channel_thumbnails.high.url) ? (
                                    <Image
                                        ...
                                        className="w-full h-full object-cover"
                                        onError={() => {
                                            failedImages.add(inf.channel_thumbnails.high.url);
                                        }}
                                    />
                                ) : (
...
const FourFoldFlipbook = ({ influencers, selectedPlatform, setSelectedPlatform, selectedTimeframe, timeframeOptions, selectedRating, setSelectedRating, searchQuery, setSearchQuery, availableRatingOptions }) => {
    ...
    const safeIndex = selectedIndex < influencers.length ? selectedIndex : 0;
    const currentInfluencer = influencers[safeIndex] || null;
    ...
    return (
        <div className="flex flex-col items-center gap-4 w-full">
            ...
            <div className="relative w-full" style={{ perspective: '2000px' }}>
                <div className="flex gap-1 md:gap-1.5 w-full" style={{ transformStyle: 'preserve-3d' }}>

                    {/* Panel 1: Influencers List */}
                    <div
                        className="flex-1 min-w-0 h-[420px] md:h-[480px] lg:h-[520px] rounded-l-xl overflow-hidden shadow-2xl bg-white"
                        style={{
                    ...
                    >
                        <TOCLeftPanel
                            ...
                            availableRatingOptions={availableRatingOptions}
                        />
                    </div>

                    {/* Panel 2: Left Panel (Filters + Profile) */}
                    <div
                        className={`flex-1 min-w-0 h-[420px] md:h-[480px] lg:h-[520px] overflow-hidden shadow-2xl transition-all duration-600 ease-in-out ${isFlipping ? 'opacity-80 scale-[0.98]' : 'opacity-100 scale-100'}`}
                        style={{
                    ...
                    <div
                        className={`flex-1 min-w-0 h-[420px] md:h-[480px] lg:h-[520px] rounded-r-xl overflow-hidden shadow-2xl transition-all duration-600 ease-in-out ${isFlipping ? 'opacity-80 scale-[0.98]' : 'opacity-100 scale-100'}`}
                        style={{
                            boxShadow: '6px 0 25px rgba(0,0,0,0.12), 0 8px 30px rgba(0,0,0,0.15)'
                        }}
                    >
...
export default function InfluencerFlipbook() {
    ...
    // Filter states per platform - remember filters independently for YouTube and Telegram
    const [platformFilters, setPlatformFilters] = useState({
        youtube: { timeframe: "180_days", rating: "all", search: "" },
        telegram: { timeframe: "180_days", rating: "all", search: "" },
    });

    // Derived filter values from current platform
    const selectedTimeframe = platformFilters[selectedPlatform].timeframe;
    const selectedRating = platformFilters[selectedPlatform].rating;
    const searchQuery = platformFilters[selectedPlatform].search;

    // Setters that update the current platform's filters
    const setSelectedRating = (value) => {
        setPlatformFilters(prev => ({
            ...prev,
            [selectedPlatform]: { ...prev[selectedPlatform], rating: value }
        }));
    };
    const setSearchQuery = (value) => {
        setPlatformFilters(prev => ({
            ...prev,
            [selectedPlatform]: { ...prev[selectedPlatform], search: value }
        }));
    };

    // Default year (same logic as influencer search page)
    ...
    useEffect(() => {
        ...
        if (selectedPlatform === "youtube" && !ytFetched) {
            ...
            fetch(`/api/telegram-data?${fixedParams}`)
                ...
                .catch(err => console.error("Failed to fetch Telegram data:", err))
                .finally(() => setLoading(false));
        }
    ...
    const filteredInfluencers = useMemo(() => {
        ...
        let influencers = rawList
            .filter(raw => {
                const isYoutube = selectedPlatform === "youtube";
                const subs = isYoutube ? raw.subs : (raw.subscribers || raw.subs || 0);
                return subs && subs > 0;
            })
            .map((raw) => {
                const isYoutube = selectedPlatform === "youtube";
                ...
                return {
                    id: channelId,
                    name: getDisplayName(raw.influencer_name, channelId),
                    ...
                    ai_scoring_yearly: raw.ai_scoring_yearly || {},
                    last_post_date: raw.last_post_date || '',
                    last_post_date_string: raw.last_post_date_string || '',
                    last_post_time: raw.last_post_time || '',
        ...
        return influencers;
    }, [selectedPlatform, youtubeInfluencers, telegramInfluencers, selectedYear, selectedTimeframe, selectedRating, searchQuery]);

    // Compute which star ratings are actually available for the current platform/year/timeframe
    const availableRatingOptions = useMemo(() => {
        const yearKey = getYearKey(selectedYear);
        const timeframeKey = selectedTimeframe;
        const rawList = selectedPlatform === "youtube" ? youtubeInfluencers : telegramInfluencers;
        ...
        rawList.forEach(raw => {
            const rating = getInfluencerRating(raw, yearKey, timeframeKey);
            if (rating > 0) {
                availableStars.add(Math.floor(rating));
            }
        });

        return [
            { value: "all", label: "All", stars: 0 },
            ...ratingOptions.filter(opt => opt.value !== "all" && availableStars.has(opt.stars))
        ];
    }, [selectedPlatform, youtubeInfluencers, telegramInfluencers, selectedYear, selectedTimeframe]);

    // Reset rating to "all" if current selection is no longer available
    useEffect(() => {
        if (selectedRating !== "all" && !availableRatingOptions.some(opt => opt.value === selectedRating)) {
            setSelectedRating("all");
        }
    }, [availableRatingOptions, selectedRating, setSelectedRating]);
