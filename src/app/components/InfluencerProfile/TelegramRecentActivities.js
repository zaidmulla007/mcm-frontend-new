  ...
  useEffect(() => {
    ...
    if (messagesData) {
      const formattedPosts = messagesData.map((message, index) => ({
        id: index + 1,
        title: message.message ? (message.message.substring(0, 50) + "...") : "No message",
    ...
    <div className="bg-white min-h-screen rounded-xl text-black p-2">
      ...
      <div className="flex gap-1 pb-2 mt-5">
        {recentPosts.map((post, index) => (
          <div
            key={post.id}
            className="w-1/5 bg-white rounded-lg overflow-hidden border border-black flex flex-col"
          >
            {/* Header with POST ID and Date */}
            <div
            ...
            <div className="p-1 border-b border-black">
              ...
              <div className="text-[9px] mb-1 font-medium" title={post.messageText}>
                {post.title}
              </div>
            ...
            <div className="p-2 border-b border-black">
              <div className="flex items-center gap-1 mb-2">
                <span className="font-bold text-[10px]">Post Summary</span>
              </div>
              ...
              {post.summary.length > 120 && (
                <button
                  ...
                  className="text-[9px] text-blue-600 hover:text-blue-800 mt-2 cursor-pointer underline font-medium"
                >
            ...
            >
              ...
              <div className="space-y-1 text-[9px]">
                {(expandedPosts[post.id]
                  ...
                  <div
                    key={i}
                    className={`flex items-start ${getSentimentColor(rec.sentiment)}`}
                    title={`${rec.coin}: ${rec.sentiment}, ${rec.term}`}
                  >
                    <span className="mr-1">*</span>
                    <span>{rec.coin}: {rec.sentiment.replace('_', ' ')}, {rec.term}</span>
                  </div>
                ))}
              </div>
                ...
                <div className="absolute top-0 left-full ml-2 z-50 bg-black text-white p-3 rounded-lg shadow-lg border border-gray-300 max-w-md">
                  <div className="text-xs">
                    ...
                    <div className="mt-2 pt-2 border-t border-gray-600">
                      ...
                      <div className="text-gray-300 text-[10px] flex items-start mt-1">
                        <span className="mr-1">*</span>
