...
import { FaBell, FaYoutube, FaTelegramPlane, FaInfoCircle, FaCertificate, FaStar, FaStarHalfAlt } from "react-icons/fa";
...
export default function YouTubeTelegramDataTable({ useLocalTime: propUseLocalTime = false }) {
    ...
    // Render stars based on star rating (already in 5-star scale)
    ...
    const renderTable = (timeframe, title) => {
        ...
        return (
            <div className="bg-gradient-to-br from-white/80 via-indigo-50/80 to-fuchsia-50/80 backdrop-blur-md rounded-3xl overflow-hidden shadow-2xl shadow-indigo-500/10 border-2 border-white/40">
                <div className="bg-gradient-to-r from-cyan-500 to-indigo-500 p-4">
                                                        ...
                                                        {timeframe === '6hrs' && isNewCoin(coin) && (
                                                            <div className="absolute -top-2 -left-8 group/newcoin z-[9999]">
                                                                <div className="relative inline-flex items-center justify-center h-6 w-6">
                                                                    ...
                                                                    <span className="absolute text-[11px] font-bold text-white uppercase tracking-tighter">N</span>
                                                                </div>
                ...
                {hasMore && (
                    <div className="flex justify-center items-center mb-2">
                        <button
                            ...
                            className="bg-gradient-to-r from-cyan-500 to-indigo-500 hover:from-cyan-600 hover:to-indigo-600 p-4 text-white font-semibold text-sm cursor-pointer rounded-lg transition-all shadow-md shadow-indigo-500/30"
                        >
    ...
    return (
        <div className="space-y-2">
            ...
            <div className="mt-4 items-start px-2">
                <h2 className="text-4xl md:text-5xl font-bold flex items-center gap-3 drop-shadow-sm">
                    <span className="bg-gradient-to-r from-cyan-500 to-indigo-500 bg-clip-text text-transparent leading-normal pb-2 inline-block">Trending Coins</span>
                    {/* <div className="relative group/mcm">
                        <FaInfoCircle
                            className="text-gray-500 text-sm cursor-pointer hover:text-indigo-600 transition-colors"
                            onClick={() => router.push('/coins-new')}
                        />
                        <div className="invisible group-hover/mcm:visible absolute top-full left-1/2 transform -translate-x-1/2 mt-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg shadow-xl whitespace-nowrap z-[9999]">
                            Click to check MCM signal view
                            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 border-4 border-transparent border-b-gray-900"></div>
                        </div>
                    </div> */}
                </h2>
                                ...
                                {Array.isArray(influencerModal.influencers) ? (
                                    influencerModal.influencers.map((influencer) => (
                                        <div
                                            ...
                                            className="flex items-center justify-between text-[10px] py-1.5 text-gray-200 hover:text-white cursor-pointer hover:bg-gray-700 px-1 rounded transition-colors"
                                            onClick={() => {
                                        ...
                                        >
                                            <span className="truncate mr-2">{influencer.influencer_name === "N/A" ? influencer.channel_id : influencer.influencer_name}</span>
                                            <div className="flex-shrink-0 text-[8px]">
                                                {renderStars(influencer["180_days_star_rating"] || 0)}
                                            </div>
                                        </div>
                                    ))
                                ) : null}
