...
const API_URL = "/api/admin/coinindex/mcmsignals";
          ...
          {Array.from({ length: maxRows }).map((_, idx) => (
            <tr key={idx} style={{ backgroundColor: idx % 2 === 0 ? '#f9fafb' : 'white' }}>
              <td style={{ padding: '12px 16px', textAlign: 'center', color: '#16a34a', fontWeight: '500' }}>
                {support?.[idx] !== undefined ? `$${Number(support[idx]).toLocaleString()}` : "-"}
              </td>
              <td style={{ padding: '12px 16px', textAlign: 'center', color: '#ef4444', fontWeight: '500' }}>
                {resistance?.[idx] !== undefined ? `$${Number(resistance[idx]).toLocaleString()}` : "-"}
              </td>
            </tr>
          ))}
        </tbody>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)' }}>
          <div style={{ backgroundColor: '#f0fdf4', padding: '8px', textAlign: 'center', borderRight: '1px solid #e5e7eb' }}>
            ...
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#16a34a', whiteSpace: 'nowrap' }}>
              {data.high52w ? `$${Number(data.high52w).toLocaleString()}` : "N/A"}
            </div>
          </div>
          <div style={{ backgroundColor: '#fef2f2', padding: '8px', textAlign: 'center' }}>
            <div style={{ fontSize: '12px', color: '#6b7280', fontWeight: '500' }}>LOW</div>
            <div style={{ fontSize: '13px', fontWeight: '700', color: '#ef4444', whiteSpace: 'nowrap' }}>
              {data.low52w ? `$${Number(data.low52w).toLocaleString()}` : "N/A"}
            </div>
          </div>
        </div>
      </div>
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        // Check sessionStorage for pre-fetched data from coins-new page
        if (coinParam) {
          const cached = sessionStorage.getItem(`mcm_report_${coinParam}`);
          if (cached) {
            data = JSON.parse(cached);
            sessionStorage.removeItem(`mcm_report_${coinParam}`);
          }
        }

        // If no cached data, fetch from API
        if (!data) {
          const fetchUrl = coinParam
            ? `${API_URL}?source_id=${encodeURIComponent(coinParam)}`
            : API_URL;
